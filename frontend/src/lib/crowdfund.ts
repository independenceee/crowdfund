...
import { SCRIPT_CBOR_SINGLE, encodeDatum, encodeRedeemer, decodeDatum, posixMsToSlot, type CampaignDatum, type Contribution } from "./contract";
...
export async function donate(wallet: BrowserWallet, campaignUtxo: UTxO, donationLovelace: bigint): Promise<string> {
    const changeAddress = await wallet.getChangeAddress();
    const utxos = await wallet.getUtxos();
    ...
    if (existingIndex >= 0) {
        newContributions = currentDatum.contributions.map((c, i) => (i === existingIndex ? { ...c, amount: c.amount + donationLovelace } : c));
    } else {
...
export async function withdraw(wallet: BrowserWallet, campaignUtxo: UTxO): Promise<string> {
    const changeAddress = await wallet.getChangeAddress();
    const utxos = await wallet.getUtxos();
...
export async function reclaim(wallet: BrowserWallet, campaignUtxo: UTxO): Promise<string> {
    const changeAddress = await wallet.getChangeAddress();
    const utxos = await wallet.getUtxos();
    const [collUTxO] = await wallet.getCollateral();
    if (!collUTxO) throw new Error("No collateral UTxO.");
