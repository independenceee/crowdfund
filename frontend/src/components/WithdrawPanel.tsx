        ...
        <div className="space-y-4">
            <div className="p-4 bg-gray-800/50 rounded-lg space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                    ...
                    <span className={isBeneficiary ? "text-teal-400" : "text-gray-500"}>{isBeneficiary ? "Beneficiary ✓" : "Not beneficiary"}</span>
                </div>
            </div>

            {!isBeneficiary && (
                <div className="p-3 bg-yellow-900/30 border border-yellow-600/40 rounded-lg">
                    <p className="text-yellow-400 text-sm">Only the beneficiary ({datum.beneficiary.slice(0, 16)}...) can withdraw.</p>
                </div>
            )}

            {!goalMet && isBeneficiary && (
                <div className="p-3 bg-orange-900/30 border border-orange-600/40 rounded-lg">
                    <p className="text-orange-400 text-sm">Goal not met yet. Need {(Number(datum.goal - total) / 1e6).toFixed(2)} more ADA.</p>
                </div>
            )}
